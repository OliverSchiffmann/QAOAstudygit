...
def create_inital_state(num_qubits, problem_type, weight_capacity=None):
    ...
    if problem_type == "TSP":
        ...
        slack_variable_encoding = f"{weight_capacity:0{3}b}"[
            ::-1
        ]  # note this only works for knapsack problems with 3 slack variables
        ...
        target_qubits = [
            index for index, char in enumerate(slack_variable_encoding) if char == "1"
        ]
        initial_circuit.x(
            target_qubits
